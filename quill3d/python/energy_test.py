...
def check(data_folder = '../results/', t=None):
    ...
    print ("W in box (t={0}) / W_0 = {1}".format(t, np.sum(data[index][1:-1]) / data[0][1]))
    if resread.catching:
        print ("W total  (t={0}) / W_0 = {1}".format(t, (np.sum(data[index][1:-1]) + np.sum(data_deleted[index_del][1:-1])) / data[0][1]))
