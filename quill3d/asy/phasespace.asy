...
// max_w, max_f, max_dndeps или max_density могут быть заданы равными нулю, в этом
...
real max_f = 5e10; /* 1/MeV, поскольку координаты нормированы на
лазерную длину волны */
real max_w = 0; // (mc\omega/e)^2
real max_dndeps = 1e11;
real epsmax = 1e10; // MeV
string plane = "xy"; // possible: xy, xz, yz, zx
string particles = "electrons"; // possible: electrons, positrons, photons
int npalette = 200;
...
else if (plane=="zx")
{
	...
	for( k=0; k<nz; k=k+1 )
	{
	    density[k][i] = csign*data[(ny+nz)*i+ny+k];
	}
    }
}
else
...
fin=input(results_folder+"w"+format("%g",file_number));
data=fin;
...
else
{
	...
	for( j=0; j<ny; j=j+1 )
	{
	    w[i][j] = data[(ny+nz)*i+j];
	}
    }
}

if (max_w==0) max_w = max(w);
range w_range = Range(0,max_w);
...
if (plane=="yz")
{
	...
	q = data[8*i];
	g = data[8*i+7];
	j = Floor(data[8*i+2]/dy);
	k = Floor(g*0.511/deps);
	if(j>-1&&j<ny&&k>-1&&k<neps) f[j][k] = f[j][k] + csign*q;
    }
    // tmp = n_cr[cm^{-3}]*dx[cm]*dy[cm]*dz[cm]/(deps*dy)
    tmp = 1.11485e13*lambda*dx*dz/deps;
}
else if (plane=="xz")
{
    for( i=0; i<n_particles; i=i+1 )
	...
	q = data[8*i];
	g = data[8*i+7];
	j = Floor(data[8*i+1]/dx);
	k = Floor(g*0.511/deps);
	if(j>-1&&j<nx&&k>-1&&k<neps) f[j][k] = f[j][k] + csign*q;
    }
    // tmp = n_cr[cm^{-3}]*dx[cm]*dy[cm]*dz[cm]/(deps*dx)
    tmp = 1.11485e13*lambda*dy*dz/deps;
}
else if (plane=="zx")
{
    for( i=0; i<n_particles; i=i+1 )
	...
	q = data[8*i];
	g = data[8*i+7];
	j = Floor(data[8*i+3]/dz);
	k = Floor(g*0.511/deps);
	if(j>-1&&j<nz&&k>-1&&k<neps) f[j][k] = f[j][k] + csign*q;
    }
    // tmp = n_cr[cm^{-3}]*dx[cm]*dy[cm]*dz[cm]/(deps*dz)
    tmp = 1.11485e13*lambda*dx*dy/deps;
}
else
{
    for( i=0; i<n_particles; i=i+1 )
...
string[] plbl2 = {"0",format("%g",max_w/2),format("%g",max_w)};
...
picture pic2;
prange=image(pic2,w,w_range,(0,0),(length1,length2),palette2);
palette(pic2,"$\left(e/mc\omega\right)^2\left({\mathbf E}^2+{\mathbf B}^2\right)$",prange,point(pic2,NW)+0.04N*length2,point(pic2,NE)+0.08N*length2,Top,palette2,PaletteTicks(new string(real x){return plbl2[Floor(2*x/max_w)];},max_w/2,max_w/4,Tickpen,tickpen));
...
frame frm2=pic2.fit();
//frm2=shift(0,min(frm1).y-max(frm1).y-0.5picturewidth*1cm)*frm2;
frm2=shift(max(frm1).x-min(frm1).x+xindent*picturewidth*1cm,0)*frm2;
add(frm2);

picture pic3;
prange=image(pic3,f,f_range,(0,0),(length1,epsmax),palette3);
palette(pic3,"$d^2N/d\varepsilon d($"+axis1_name+"$)$",prange,point(pic3,NW)+0.04N*epsmax,point(pic3,NE)+0.08N*epsmax,Top,palette3,PaletteTicks(new string(real x){return plbl3[Floor(2*x/max_f)];},max_f/2,max_f/4,Tickpen,tickpen));
xlimits(pic3,0,length1);
ylimits(pic3,0,epsmax);
xaxis(pic3,axis1_name,BottomTop,LeftTicks,true);
yaxis(pic3,"$\varepsilon$, MeV",LeftRight,RightTicks,true);
size(pic3,picturewidth*1cm,picturewidth*1cm*length2/length1,point(pic3,SW),point(pic3,NE));
frame frm3=pic3.fit();
//frm3=shift(max(frm1).x-min(frm1).x+0.7picturewidth*1cm,0)*frm3;
frm3=shift(0,min(frm1).y-max(frm1).y-yindent*picturewidth*1cm)*frm3;
add(frm3);

pen p_line = linejoin(2)+linewidth(1.0);
pen p_dndeps = p_line + 0.8*red+0.8*blue;
picture pic4;
draw(pic4,graph(dndeps,eps,Straight),p_dndeps);
xlimits(pic4,0,max_dndeps,Crop);
ylimits(pic4,0,epsmax);
xaxis(pic4,"$dN/d\varepsilon$",BottomTop,LeftTicks,true);
yaxis(pic4,"$\varepsilon$, MeV",LeftRight,RightTicks,true);
